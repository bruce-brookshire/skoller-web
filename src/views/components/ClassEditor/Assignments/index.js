...
class Assignments extends React.Component {
  ...
  async componentWillMount () {
    ...
    await actions.weights
      ...
      .then(() => {
        this.setState({ loadingWeights: false })
      })
  ...
  onCreateAssignment (assignment) {
    assignment.due
      ? this.updateLastAssignmentDate(
        moment(assignment.due).format('MM/DD/YYYY')
      )
  ...
  onNext () {
    ...
    if (weights[currentWeightIndex + 1]) {
      this.setState({
        ...
        currentWeight: weights[currentWeightIndex + 1],
        addAssignment: true
      })
    } else {
      this.props.onSubmit()
  ...
  toAssignmentForm (weight) {
    this.setState({
      addAssignment: true,
      currentWeight: weight,
      currentWeightIndex: this.state.weights.indexOf(weight)
    })
  }

  updateLastAssignmentDate (date) {
    if (date) this.setState({ lastAssignmentDate: date })
  }
  ...
  async submitAssignments () {
    ...
    showSnackbar(
      `Created ${assignmentCount} new assignment` +
        (assignmentCount > 1 ? 's' : ''),
      'success'
    )
    this.setState({ loadingAssignments: true })
    await actions.assignments
      .getClassAssignments(this.props.cl)
      .then((assignments) => {
        this.setState({ assignments, loadingAssignments: false })
      })
      .then(() => {
  ...
  renderProgressBar () {
    return (
      <div className="cn-section-progress-outer">
        <img
          alt="Skoller"
          className="logo"
          ...
          <div className="infodiv">
            ...
            >
              <div className="tooltipBox">
                Add all graded assignments for this class <br></br>
                <br></br>
                Tip 1: If the due date is unknown but the assignment is sure to
                happen, go ahead and add it<br></br>
                <br></br>
                Tip 2: You can always add and edit assignments during the
                semester
              </div>
            </ReactTooltip>
          </div>
        ...
        <div className="cn-pull-right">
          ...
          <span className="cn-section-progressbar">
            {' '}
            <ProgressBar percent={(2 / 3) * 100} />
          </span>
          <a
            className="cn-section-icons"
            onClick={() => this.onUpdateCurrentIndex({ currentIndex: 0 })}
          >
            <i className="fas fa-angle-left"></i>
          </a>
          <a
            className="cn-section-icons"
            onClick={() => this.onUpdateCurrentIndex({ currentIndex: 2 })}
          >
            ...
            {!viewOnly && (
              <div id="class-editor-assignment-form">
                ...
                <AssignmentForm
                  ...
                  currentWeight={currentWeight}
                  updateLastAssignmentDate={(date) =>
                    this.updateLastAssignmentDate(date)
                  }
                  lastAssignmentDate={this.state.lastAssignmentDate}
                  toggleAddingAssignment={(bool) =>
                    this.toggleAddingAssignment(bool)
                  }
                />
              </div>
            )}
              ...
              <div id="cn-assignment-table-new">
                <div className="cn-assignment-notadded">
                  <div className="center-text ">
                    ...
                    />
                    <h2>
                      Add all assignments that will count towards your final
                      grade in this class!
                    </h2>
                    <h4>
                      Note: leave due dates empty if assignments don't have
                      specific due dates at this time.
                    </h4>
                  </div>
                </div>
              </div>
