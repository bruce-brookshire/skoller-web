...
class ChangeSchool extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      schoolChoice: this.props.backData
        ? this.props.backData.schoolChoice
        : null,
  ...
  renderAutoComplete(school) {
    ...
    return this.state.loadingAutocomplete ? null : (
      <div
        style={{
  ...
  renderSchool() {
    return (
      <div className="sk-select-school">
        ...
        {(this.state.schools.length > 0 || this.state.input) &&
        !this.state.loadingAutocomplete ? (
          <div
            className="sk-select-school-autocomplete-container"
            style={{ width: this.schoolField.offsetWidth.toString() + "px" }}
        ...
        {this.state.schoolChoice ? (
          <div className="sk-select-school-row">{this.renderTermField()}</div>
        ) : null}
      </div>
    );
  }
  ...
  renderSchoolChoice() {
    return (
      <div className="sk-select-school-selected-school">
        <h3
          className="sk-select-school-selected-school-name"
          style={{
            color: this.state.schoolChoice.color
              ? this.state.schoolChoice.color
              : null,
          }}
        >
        ...
        <div className="sk-select-school-selected-school-location">
          {this.state.schoolChoice.adr_locality},{" "}
          {this.state.schoolChoice.adr_region}
        </div>
      </div>
    );
  }
  ...
  setSchoolChoice = (school) => {
    this.setState({
      schoolChoice: school,
      activeTerm: this.findActiveTerm(school),
      termChoice: this.findActiveTerm(school),
    });
  };
  ...
  renderTermField() {
    return (
      <div className="sk-select-school-term">
        <div className="sk-select-school-term-label">Select Term</div>
        {this.state.termChoice ? (
          <div
            className="sk-select-school-term-field"
            onClick={() =>
              this.setState({ showTermOptions: !this.state.showTermOptions })
            }
            ref={(termField) => {
          ...
          >
            <div
              key={this.state.termChoice.id}
              className="sk-select-school-term-choice"
            >
              <h3>{this.state.termChoice.name}</h3>
              <div className="sk-select-school-term-dates">
                {moment(this.state.termChoice.start_date).format("MMMM")} -{" "}
                {moment(this.state.termChoice.end_date).format("MMMM")}
              </div>
            </div>
          </div>
          ...
          >
            <div
              key={this.state.activeTerm.id}
              className="sk-select-school-term-choice"
            >
              <h3>{this.state.activeTerm.name}</h3>
              <div className="sk-select-school-term-dates">
                {moment(this.state.activeTerm.start_date).format("MMMM")} -{" "}
                {moment(this.state.activeTerm.end_date).format("MMMM")}
              </div>
            </div>
          </div>
        ) : (
        ...
        {this.state.showTermOptions ? this.renderTermOptions() : null}
      </div>
    );
  }
  ...
  renderTermOptions() {
    ...
    return (
      <div className="sk-select-school-term-options">
        {currentTerms.map((term) => {
          return (
            ...
            >
              <h3>{term.name}</h3>
              <div className="sk-select-school-term-dates">
                {moment(term.start_date).format("MMMM")} -{" "}
                {moment(term.end_date).format("MMMM")}
              </div>
            </div>
          );
        })}
  ...
  async onSubmitSchool() {
    ...
    await actions.students
      .setStudentPrimarySchool(userId, studentId, this.state.schoolChoice.id)
      .catch((e) => console.log(e));
    await actions.students
      .setStudentPrimaryPeriod(userId, studentId, termChoice.id)
      .catch((e) => console.log(e));
    await actions.users.refreshUser().catch((e) => console.log(e));
    this.props.onSubmit({
      termChoice: this.state.termChoice
        ? this.state.termChoice
        : this.props.backData.termChoice,
      schoolChoice: this.state.schoolChoice
        ? this.state.schoolChoice
        : this.props.backData.schoolChoice,
    });
  }
  ...
  render() {
    ...
    if (
      (!this.state.termChoice || !this.state.activeTerm) &&
      !this.state.schoolChoice
    ) {
      disableNext = true;
    }
    return (
      <div>
        {this.state.loading ? <SkLoader /> : null}
        {this.state.loading ? null : (
          <div>
            ...
            {this.state.showCreateSchoolModal ? (
              <SkModal
                closeModal={() =>
                  this.setState({ showCreateSchoolModal: false })
                }
              >
                <CreateSchoolModal
